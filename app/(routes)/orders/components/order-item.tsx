...
const OrderItem = ({ order }: OrderItemProps) => {
  ...
  return (
    <Box>
      <div className="w-full grid grid-cols-2 items-center md:grid-cols-4 gap-x-4 gap-y-6 px-4 py-2 rounded-md border border-gray-100">
        ...
        <p
          className={cn(
            "text-base font-semibold flex items-center gap-40",
            order.isPaid ? "text-emerald-500" : "text-red-500"
          )}
        >
          {order.isPaid ? "Paid" : "Not Paid"}

          <Radar className="cursor-pointer text-neutral-500" onClick={handleTrackOrder}/>
        </p>
      </div>
    </Box>
  );
